...
export function filterItems(
  ...
  return items.filter((item) => {
    if (healthFilter.size > 0 && !healthFilter.has(item.health || "unknown"))
      return false;
...
export function globalSearch(
  ...
  for (const e of har || []) {
    if (
      containsCI(e.request.url, lq) ||
      containsCI(e.request.method, lq) ||
  ...
  n = 0;
  for (const u of results?.external_users || [])
    if (containsCI(u.name, lq) || u.aliases?.some((a) => containsCI(a, lq)))
      n++;
  if (n) counts.users = n;

  n = 0;
  for (const g of results?.external_groups || [])
    if (containsCI(g.name, lq) || g.aliases?.some((a) => containsCI(a, lq)))
      n++;
  if (n) counts.groups = n;

  n = 0;
  for (const r of results?.external_roles || [])
    if (containsCI(r.name, lq) || r.aliases?.some((a) => containsCI(a, lq)))
      n++;
  if (n) counts.roles = n;
  ...
  for (const a of results?.config_access || [])
    if (
      a.external_user_aliases?.some((x) => containsCI(x, lq)) ||
  ...
  for (const ch of results?.changes || [])
    if (
      containsCI(ch.summary, lq) ||
      containsCI(ch.change_type, lq) ||
      containsCI(ch.diff, lq) ||
      containsCI(ch.external_created_by, lq)
    )
      n++;
...
export function matchesSearchArr(
  q: string,
