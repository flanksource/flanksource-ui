...
function matchesEntity(
  ...
  const targets =
    kind === "user"
      ? access.external_user_aliases
      : kind === "group"
        ? access.external_group_aliases
...
function columnsFor(
  ...
  if (kind === "role")
    base.splice(1, 0, { key: "aliases", label: "Aliases", cls: "px-3 py-2" });
  if (kind === "user")
    base.push({ key: "groups", label: "Groups", cls: "px-3 py-2" });
  if (kind === "group")
    base.push({ key: "members", label: "Members", cls: "px-3 py-2" });
  return base;
}
...
export function EntityTable({
  ...
  const filtered = useMemo(() => {
    ...
    return entities.filter((e) =>
      matchesSearch(search, e.name, ...(e.aliases || []))
    );
  }, [entities, search]);
  ...
  const resolveMembership = useCallback(
    (ug: ExternalUserGroup): { userId?: string; groupId?: string } => {
      let userId = ug.external_user_id;
      if (!userId && ug.external_user_aliases?.length && allUsers) {
        const u = allUsers.find((x) =>
          ug.external_user_aliases!.some(
            (a) => a === x.id || x.aliases?.includes(a)
          )
        );
        if (u) userId = u.id;
      }
      ...
      if (!groupId && ug.external_group_aliases?.length && allGroups) {
        const g = allGroups.find((x) =>
          ug.external_group_aliases!.some(
            (a) => a === x.id || x.aliases?.includes(a)
          )
        );
        if (g) groupId = g.id;
  ...
  const resolvedUserGroups = useMemo(() => {
    ...
    return userGroups
      ...
      .filter((r) => r.userId && r.groupId) as {
      userId: string;
      groupId: string;
    }[];
  }, [userGroups, resolveMembership]);
          ...
          <tbody>
            {sorted.map((e, idx) => (
              <tr
                ...
                onClick={() =>
                  onSelect?.(selectedId === e.id ? undefined : e.id)
                }
              >
                ...
                {(kind === "user" || kind === "group") && (
                  <td className="px-3 py-2">
                    {membershipCounts[e.id] ? (
                      <span
                        className={`rounded px-1.5 py-0.5 text-xs ${kind === "group" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"}`}
                      >
                        {membershipCounts[e.id]}
                      </span>
                    ) : (
