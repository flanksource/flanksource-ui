  ...
  const statusLines = useMemo(() => {
    ...
    return (
      [healthSummary, healthChecks, insights, ...incidents, ...components]
        ...
        .sort((a, b) => {
          if (a?.statuses.some((status) => status.color === "red")) {
            return -1;
          }
          if (b?.statuses.some((status) => status.color === "red")) {
            return 1;
          }

          if (a?.statuses.some((status) => status.color === "orange")) {
            return -1;
          }
          if (b?.statuses.some((status) => status.color === "orange")) {
            return 1;
          }
