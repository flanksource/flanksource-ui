import { useComponentConfigRelationshipQuery } from "@flanksource-ui/api/query-hooks/useComponentConfigRelationshipQuery";
...
import {
  getSortedTopology,
  getSortLabels
} from "./TopologyPopover/topologySort";
...
type TopologyPageProps = {
  catalogTab: ConfigTab;
};

export function TopologyPageWrapper({
  catalogTab = "Catalog"
}: TopologyPageProps) {
  const { id } = useParams();
  ...
  const { data: topologyConfigs, isLoading: isLoadingConfigs } =
    useComponentConfigRelationshipQuery({
      topologyId: id,
      hideDeleted: true
    });

  const isLoading = isLoadingTopology || isLoadingConfigs;

  const currentTopology = useMemo(() => data?.components?.[0], [data]);

  const showMergedTopologyConfigPage = useMemo(() => {
    return topologyConfigs?.data?.length === 1;
  }, [topologyConfigs?.data?.length]);
        ...
        <div className="flex h-full flex-row overflow-y-auto py-2">
          <div className="flex h-full flex-1 flex-col overflow-y-auto">
            ...
            {isLoading && !topology?.length ? (
              <CardsSkeletonLoader />
            ) : showMergedTopologyConfigPage ? (
              <MergedTopologyConfigPage
                topologies={topology ?? []}
                children={undefined}
                activeTabName={catalogTab}
                configId={topologyConfigs?.data?.[0]?.config_id!}
              />
            ) : (
