...
export async function getChildPlaybookRuns(parentId: string) {
  const select = [
    "*",
    "created_by(id,name,avatar)",
    "playbooks(id,name,title,spec,icon)",
  ...
  const res = await IncidentCommander.get<PlaybookRunWithActions[] | null>(
    `/playbook_runs?parent_id=eq.${parentId}&select=${select}`
  );
  ...
  // Fetch actions for each child run
  const childRuns = await Promise.all(
    res.data.map(async (run) => {
      const resActions = await IncidentCommander.get<
        PlaybookRunAction[] | null
      >(
        `/playbook_run_actions?select=${actionsSelect}&order=start_time.asc&playbook_run_id=eq.${run.id}`
      );
